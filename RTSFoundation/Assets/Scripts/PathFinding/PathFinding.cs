...
public class PathFinding : MonoBehaviour
{
        ...
        if (startNode.walkable && targetNode.walkable)
        {
            ...
            while (openSet.Count > 0)
            {
                ...
                foreach (MapNode neighbor in grid.GetNeighbors(current))
                {
                    ...
                    // Testing this to see if it produces better paths and how much overhead is added.
                    if (!Walkable(current.worldPosition, neighbor.worldPosition, checkSphereRadius))
                        continue;
    ...
    Vector3[] SmoothPath(List<MapNode> path)
    {
        ...
        for (int i = 1; i < path.Count; i++)
        {
            ...
            if (dyNew - dyOld >= epsilon || !Walkable(smoothedPath[smoothedPath.Count - 1], path[i].worldPosition, checkSphereRadius))
            {
    ...
    {
        // First first check that B is walkable
        if (Physics.CheckSphere(B + Vector3.up * (checkRadius + Grid.Instance.maxStepHeight), checkRadius, obsticleLayer))
            return false;
        ...
        Vector3 checkPoint = A + Vector3.up * (checkRadius + Grid.Instance.maxStepHeight);
        while (Mathf.Abs(checkPoint.x - B.x) > checkRadius || Mathf.Abs(checkPoint.z - B.z) > checkRadius)
        {
            ...
            {
                if (checkPoint.y - rHit1.point.y > Grid.Instance.maxStepHeight + checkRadius + sphereColliderBuffer || checkPoint.y - rHit2.point.y > Grid.Instance.maxStepHeight + checkRadius + sphereColliderBuffer)
                    return false;
            }
    ...
    IEnumerator DebugSmoothPath(List<MapNode> path)
    {
        ...
        for (int i = 1; i < path.Count; i++)
        {
            print("i: " + i);
            float dyNew = path[i].worldPosition.y - path[i-1].worldPosition.y;
            yield return StartCoroutine(DebugWalkable(_smoothPath[_smoothPath.Count - 1], path[i].worldPosition, checkSphereRadius));
    ...
    IEnumerator DebugWalkable(Vector3 A, Vector3 B, float checkRadius)
    {
        ...
        checkPoint = B + Vector3.up * (checkRadius + Grid.Instance.maxStepHeight);
        if (Physics.CheckSphere(checkPoint, checkRadius, obsticleLayer))
        {
            _valid = false;
            invalidWalkPoints.Add(checkPoint);
    ...
    private void OnDrawGizmos()
    {
        if (debugMode)
        {
            ...
            {
                Gizmos.DrawSphere(point, checkSphereRadius);
            }
