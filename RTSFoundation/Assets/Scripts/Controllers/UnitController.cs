...
public class UnitController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameCtrlV2.Instance.RegisterActiveUnit(this);
    }

    // for debugging
    bool _flagMessage = false;
    void Update()
    {
        if (currentPath != null && currentPath.RemainingDistanceInPath <= 0 && !_flagMessage)
        {
            print("Finished total distance; " + gameObject.name);
            _flagMessage = true;
        }
    }
    ...
    {
        GameCtrlV2.Instance.UnregisterActiveUnit(this);
        Destroy(gameObject);
    }

    void RegisterWithOrdersCtrl()
    {
        UnitOrdersCtrlV2.Instance.moveUnitsConvergeOnPoint += MoveConvergeOnPoint;
        UnitOrdersCtrlV2.Instance.moveUnitsKeepFormation += MoveKeepFormation;
    }
    ...
    void OnPathFound(Vector3[] _path, bool success)
    {
        if (success && _path.Length > 0)
        {
            ...
            print(Walkable(navPoint.position, _path[1], 0.25f));
