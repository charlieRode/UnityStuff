    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for ( int y = 0; y < gridSizeY; y++)
            {
                ...
                {
                    bool walkable = !Physics.CheckSphere(hit.point, nodeRadius, unwalkable);
                    walkableRegionsDict.TryGetValue(hit.collider.gameObject.layer, out nodeWeight);
                    if (!walkable)
                        nodeWeight += obsticleProximityPenalty;

                    grid[x, y] = new Node(walkable, hit.point, x, y, nodeWeight);
                }
                else
                {
                    grid[x, y] = new Node(false, Vector3.zero, -1, -1, nodeWeight);
                }
            }
        }
